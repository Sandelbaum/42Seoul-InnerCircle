...
/*   Updated: 2022/07/11 19:17:29 by kwolee           ###   ########seoul.kr  */
/*                                                                            */
/* ************************************************************************** */
...
static char	*read_fd(t_fd_list *node)
{
	...
	r_status = 0;
	str = node->str;
	while (str == NULL || ft_gnl_strchr(str, '\n') == NULL)
	{
...
static char	*get_one_line(t_fd_list *node)
{
	...
	if (ft_gnl_strchr(node->str, '\n') != NULL)
		len = ft_gnl_strchr(node->str, '\n') - node->str + 1;
	else
		len = ft_gnl_strchr(node->str, '\0') - node->str;
	str = (char *)malloc(len + 1);
	if (str == NULL)
		return (NULL);
	ft_gnl_strlcpy(str, node->str, len + 1);
	left = (char *)malloc(ft_gnl_strlen(node->str + len) + 1);
	if (left == NULL)
	{
		free(str);
		return (NULL);
	}
	ft_gnl_strlcpy(left, node->str + len, ft_gnl_strlen(node->str + len) + 1);
	free(node->str);
	node->str = left;
	return (str);
}
