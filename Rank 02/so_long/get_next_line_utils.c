...
/*   Updated: 2022/07/12 22:24:44 by kwolee           ###   ########seoul.kr  */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_gnl_strjoin(char const *str, char const *buf)
{
	...
	if (buf == NULL)
		return (NULL);
	buf_len = ft_gnl_strlen(buf);
	if (str == NULL)
	{
		j_str = (char *)malloc(buf_len + 1);
		if (j_str == NULL)
			return (NULL);
		ft_gnl_strlcpy(j_str, buf, buf_len + 1);
		return (j_str);
	}
	str_len = ft_gnl_strlen(str);
	j_str = (char *)malloc(str_len + buf_len + 1);
	if (j_str == NULL)
		return (NULL);
	ft_gnl_strlcpy(j_str, str, str_len + 1);
	ft_gnl_strlcpy(j_str + str_len, buf, buf_len + 1);
	return (j_str);
}
